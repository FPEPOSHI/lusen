...
- **`lusen:check` reports a FormRequest it read no fields from.** The two
  failures above were silent for a day because nothing asks whether an
...
- **A rule string built by concatenation lost the whole field.** `'required|
  string|max:'.self::LIMIT` and `'in:'.implode(',', self::TYPES)` are the
  ordinary way a limit reaches a rule, and neither is a literal, so the field
  they belonged to was dropped rather than typed. Class constants are read
